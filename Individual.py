...
class Individual(object):
    ...
    def __init__(self):
        self.values = numpy.zeros((board_size, board_size), dtype=int)
        self.fitness = None
        return
    ...
    def update_fitness(self):
        ...
        row_count = numpy.zeros(board_size)
        column_count = numpy.zeros(board_size)
        block_count = numpy.zeros(board_size)
        row_sum = 0
        column_sum = 0
        block_sum = 0

        for i in range(0, board_size):  # For each row...
            for j in range(0, board_size):  # For each number within it...
                row_count[self.values[i][j]-1] += 1  # ...Update list with occurrence of a particular number.

            row_sum += (1.0/len(set(row_count)))/board_size
            row_count = numpy.zeros(board_size)

        for i in range(0, board_size):  # For each column...
            for j in range(0, board_size):  # For each number within it...
                column_count[self.values[j][i]-1] += 1  # ...Update list with occurrence of a particular number.

            column_sum += (1.0 / len(set(column_count)))/board_size
            column_count = numpy.zeros(board_size)
        ...
        for i in range(0, board_size, 3):
            for j in range(0, board_size, 3):
                ...
                block_sum += (1.0/len(set(block_count)))/board_size
                block_count = numpy.zeros(board_size)
